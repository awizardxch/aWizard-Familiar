...
Exit 2 means nothing was exercised (build outputs or corpus absent); it is not a pass.
...
**Counting a pool's generations by hint:** the deployer's rename coins are hinted with the launcher id too (that is how names resolve), sit at the deployer's puzzle hash and may hold 1 mojo; drop coins at the deployer's puzzle hash before counting singletons, and drop `rename` history entries before counting spends.
