...
- **The pins are the rue builds.** chia-sdk-types 0.36.0 ships the tree hashes of
  ...
  **rue 0.8.4 reproduces every hash** — `contracts/_test_v11_integrity.py` recompiles the vendored
  sources in a scratch copy on every run to prove it.
- **V11 is a rue project**, `contracts/v11/` (`Rue.toml`: entrypoint `puzzles`, dist `compiled`).
...
- **The curve is a module, not a puzzle.** `puzzles/forge_curve.rue` exports the V10 functions over
  ...
  compile byte-identical to V10, and `_test_v11_curve_equivalence.py` holds the rest to identical
...
- **The multi-reserve finalizer exists and is probed.** `contracts/v11/puzzles/forge_multi_reserve_finalizer.rue`
  ...
  `_test_v11_finalizer.py` (42 checks) runs it through `chia_rs.get_conditions_from_spendbundle`, the
...
- **V11 is live on testnet11** (2026-09-05): registry `31ab7ead7b06d698…`, pool `be4c270006a43e3f…`, every leaf
  confirmed via `scripts/deploy-v11-testnet.py`; `forge_v11_driver.py` is the one implementation both the
...
- **Multi-action ordering.** The action layer prepends each action's condition list, so the finalizer
  ...
  reversed, concatenated in execution order. `forge_v11_driver.spend_actions` mirrors this and the
...
- **Launch matrix live**: 16 registered V11 pools on testnet11 (2026-09-05), record in
  `.awizard/v11-testnet.json`; vaults, nested LP reserves, weighted, three- and five-asset, fee edges.
- **Lifecycle across the matrix is on chain** (71 confirmed transactions on 2026-09-05): adds on all pools,
  ...
  per pool balance on their own. `scripts/v11_ops.py` is the generic operator; `resync` re-runs a spend's
...
- **Keyless settlement** (`contracts/forge_v11_offer.py`, behind `forge_stdin.py` for V11 snapshots, live
...
- **Resync by replay** (`contracts/forge_v11_resync.py`): a V11 tip's state is curried, so a lagging snapshot
...
- **Routes as hubs** (`contracts/forge_v11_route.py`, live 2026-09-05): per asset, producers (offer settlements,
